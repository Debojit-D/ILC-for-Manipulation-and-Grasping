...
start_x = 0.1096  # Starting x-coordinate
end_x = 0.1096  # Ending x-coordinate
start_z = 0.1358
end_z = 0.1858
...
def compute_end_effector_forces(k):
    ...
    if all(value is not None for value in [latest_data['torques'], latest_data['jacobian'], latest_data['g_matrix']]):
        ...
        latest_data['external_forces'] = [end_effector_forces[0], end_effector_forces[2]]
...
def trial_loop():
    ...
    for n in range(trials):
        ...
        for k in range(0, n_samples-1):
            ...
            next_TE = np.matmul(A, TE[:, [k]]) + np.matmul(B, U[:, k:k+1])
