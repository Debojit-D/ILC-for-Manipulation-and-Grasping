...
def send_pose_home():
    ...
    try:
        ...
        target_pose = Pose()
        target_pose.position = Point(x=0.09, y=0.0, z=0.06)
        roll, pitch, yaw = 0, 1.3588, 0
...
start_x = 0.09  # Starting x-coordinate
end_x = 0.15  # Ending x-coordinate
start_z = 0.06
end_z = 0.06
...
def generate_poses(start_x, end_x, z_coordinate, num_points, y_constant):
    x_traj, z_traj = minimum_jerk(start_x, end_x, start_z, end_z, num_points)
    return [Point(x=x, y=y_constant, z=z) for x, z in zip(x_traj, z_traj)]
...
latest_data = {
    ...
    'jacobian': None,
    'g_matrix': None,
    'desired_trajectory': np.vstack((xd, zd)),  # Initialize with the desired trajectory
...
def compute_end_effector_forces(k):
    ...
    if all(value is not None for value in [latest_data['torques'], latest_data['jacobian'], latest_data['g_matrix']]):
        ...
        latest_data['external_forces'] = [end_effector_forces[0], end_effector_forces[1]]  # x and z
