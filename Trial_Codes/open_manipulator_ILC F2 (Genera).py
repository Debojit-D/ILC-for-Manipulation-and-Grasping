#### This version of the code actually has one fundamental flaw and that is the section for compute end effector_forces 
...
def send_pose_home():
    ...
    try:
        ...
        target_pose = Pose()
        target_pose.position = Point(x=0.1526, y=0.0, z=1822)
...
start_x = 0.1526  # Starting x-coordinate
end_x = 0.1526  # Ending x-coordinate
start_z = 0.1822
end_z = 0.2022
y_coordinate = 0.00 # Constant z-coordinate
...
TE = np.zeros((2, n_samples))  # Error tracking
...
def generate_poses(start_x, end_x, start_z, end_z, num_points, y_coordinate):
...
yd = np.ones(n_samples) * y_coordinate  # Desired trajectory in y (constant)
zd = np.ones(n_samples) * zd  # Desired trajectory in z (constant)
XD = np.vstack((xd,yd))
end_effector_positions = np.array([[start_x] * n_samples, [start_z] * n_samples])
...
def compute_end_effector_forces(k):
    ...
    if all(value is not None for value in [latest_data['torques'], latest_data['jacobian'], latest_data['g_matrix']]):
        ...
        #print(end_effector_forces[2])
        latest_data['external_forces'] = np.array([end_effector_forces[0], end_effector_forces[2]])
...
def force_wrapper(force_measured, f_previous):
    ...
    f_hat_x= (np.tanh(x**14))*x
    f_hat_z= (np.tanh(x**14))*z
    
    # Update both x and y components of the force vector based on the previous values
    f_x = f_previous[0] + 0.8 * (f_hat_x - f_previous[0])
    f_z = f_previous[1] + 0.8 * (f_hat_z - f_previous[1])  # Update y-component based on its own current and previous value
...
def sat_u(v, saturation_min=-100, saturation_max=100):
    return np.clip(v, saturation_min, saturation_max)

def trial_loop():
    
    global X, u_ilc, U, latest_data, end_effector_forces_trial_array, f_previous, TE
    ...
    for n in range(trials):
        ...
        X, u_ilc, U, TE = controller(XD, end_effector_positions, u_ilc, U)
        ...
        for k in range(0, n_samples-1):
            ...
            next_TE = np.matmul(A, TE[:, [k]]) + np.matmul(B, (U[:, k:k+1]+eff_expanded))
            TE[:, k+1] = np.squeeze(next_TE)
