...
def send_pose_home(x, y, z, Rx, Ry, Rz, path_time):
    """
    Sends a pose to the robot using the ROS service.

    Parameters:
    - x, y, z: Coordinates of the position in meters.
    - Rx, Ry, Rz: Orientation in radians (roll, pitch, yaw).
    - path_time: Time in seconds for the robot to reach the pose.
    """
...
def trial_loop():
    ...
    for n in range(trials):
        ...
        for k in range(n_samples):
            ...
            rospy.sleep(dt)
        rospy.sleep(3)
        print(latest_data['end_effector_position'])
        send_pose_home(latest_data['end_effector_position'][0], latest_data['end_effector_position'][1], 0.30, 0.0, 0.75, 0.16, 2)
        send_pose_home(0.1, latest_data['end_effector_position'][1], 0.16, 0.0, 0.75, 0.16, 2)
        send_pose_home(0.1, 0.0, 0.16, 0.0, 0.75, 0.16, 2)
