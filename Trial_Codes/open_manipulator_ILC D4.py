...
def send_pose_home():
    ...
    try:
        ...
        target_pose = Pose()
        target_pose.position = Point(x=0.10, y=0.0, z=0.20)
...
start_x = 0.10  # Starting x-coordinate
end_x = 0.18  # Ending x-coordinate
start_y = 0.00
end_y = 0.00
z_coordinate = 0.20 # Constant z-coordinate
...
def sat_u(v, saturation_min=-200, saturation_max=200):
...
def trial_loop():
    ...
    for n in range(trials):
        ...
        if n + 1 == 37:  # Check if it's trial number 35
            rospy.loginfo("Starting extended delay for Trial 35.")
            rospy.sleep(120)  # 2-minute delay
