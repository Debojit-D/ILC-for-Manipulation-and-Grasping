...
end_effector_forces = np.zeros((2, n_samples))


################################################################## ESSENTIAL FUNCTIONS #######################################################################

# Controller function placeholders
def sat_u(v, saturation_min=-20, saturation_max=20):
...
def joint_state_callback(data):
    # Extract the torques for the first four joints, excluding the gripper
    if len(data.effort) >= 5:  # Check if the efforts array is long enough
        torques = np.array(data.effort[:4])  # Only take the first four values
        update_data_storage('torques', torques)
    else:
...
def compute_end_effector_forces():
    ...
    if all(value is not None for value in [latest_data['torques'], latest_data['jacobian'], latest_data['g_matrix']]):
        ...
        #print(latest_data['external_forces'])
...
def trial_loop():
    ...
    for n in range(trials):
        # Update the measured trajectory XM in the controller function
        X, u_ilc, U = controller(XD, end_effector_positions, u_ilc, U, t, end_effector_forces)

        for k in range(n_samples):
            # Create a Point for the current sample
            pose = Point(x=X[0, k], y=X[1, k], z=z_coordinate)
            # Send this pose to the robot
            send_pose([pose], dt)  # Ensure send_pose takes a list of Point objects and path_time
            ...
            if 'end_effector_position' in latest_data and latest_data['end_effector_position'] is not None:
                end_effector_positions[:, k] = latest_data['end_effector_position'][:2]
            else:
                end_effector_positions[:, k] = [0.0, 0.0]  # Fallback in case the position is not received

            # Store the current end-effector forces
            if 'external_forces' in latest_data and latest_data['external_forces'] is not None:
                end_effector_forces[:, k] = latest_data['external_forces'][:, k]
            else:
                end_effector_forces[:, k] = [0.0, 0.0]  # Fallback in case the forces are not received
