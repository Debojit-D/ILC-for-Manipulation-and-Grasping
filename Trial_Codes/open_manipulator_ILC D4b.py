...
def send_pose_home():
    ...
    try:
        ...
        target_pose = Pose()
        target_pose.position = Point(x=0.08, y=0.0, z=0.22)
...
def calculate_rotation_angle(start_x, start_y, end_x, end_y, end_x_new, end_y_new):
    # Calculate the original and new vectors
    vector_original = np.array([end_x - start_x, end_y - start_y])
    vector_new = np.array([end_x_new - start_x, end_y_new - start_y])
    
    # Calculate the angles of these vectors with respect to the x-axis
    angle_original = np.arctan2(vector_original[1], vector_original[0])
    angle_new = np.arctan2(vector_new[1], vector_new[0])
    
    # The rotation angle needed to align the original vector to the new vector
    rotation_angle = angle_new - angle_original
    
    return rotation_angle

def apply_rotation(X, angle):
...
end_x = 0.18  # Ending x-coordinate
start_y = 0.00
end_y = 0.00
z_coordinate = 0.22 # Constant z-coordinate
...
def trial_loop():
    ...
    global X, u_ilc, U, latest_data, end_effector_forces_trial_array, f_previous, XD
    
    for n in range(trials):
        
        rospy.loginfo("Trajectroy Needs to be Rotated")

        if n == 5:  # Change this condition to your desired iteration
            rospy.loginfo("Trajectory Needs to be Rotated")
            # Change the end point of the trajectory
            end_x_new = 0.15  # New end x-coordinate
            end_y_new = 0.1  # New end y-coordinate

            # Calculate the rotation angle based on the new end point
            rotation_angle = calculate_rotation_angle(start_x, start_y, end_x, end_y, end_x_new, end_y_new)

            # Apply the rotation matrix to the state X
            X = apply_rotation(X, rotation_angle)
            XD = apply_rotation(XD, rotation_angle)
