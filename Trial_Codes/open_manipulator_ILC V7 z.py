...
def send_pose_home():
    ...
    try:
        ...
        target_pose = Pose()
        target_pose.position = Point(x=0.09, y=0.0, z=0.19)
        roll, pitch, yaw = 0, 0.8, 0
        ...
        if response.is_planned:
            #rospy.https://chatgpt.com/c/3b903efd-35c3-410e-8a5f-119581df5c79loginfo(" Home pose successfully sent to the robot.")
            pass
        else:
...
end_x = 0.22  # Ending x-coordinate
start_y = 0.00
end_y = 0.00
z_coordinate = 0.19  # Constant z-coordinate
...
def compute_end_effector_forces(k):
    ...
    if all(value is not None for value in [latest_data['torques'], latest_data['jacobian'], latest_data['g_matrix']]):
        ...
        latest_data['external_forces'] = [end_effector_forces[0], end_effector_forces[1]]
